...
from braindecode.modules import Expression, Ensure4d, SqueezeFinalOutput, SafeLog

class CustomEEGNet(EEGModuleMixin, nn.Sequential):
    def __init__(self, in_chans, input_window_samples, n_classes):
        super().__init__(
            Ensure4d(),
            nn.Conv2d(1, 40, (1, 25), stride=1),
            nn.Conv2d(40, 40, (in_chans, 1), stride=1, groups=40),
            nn.BatchNorm2d(40),
            ...
            nn.Dropout(p=0.5),
            nn.Flatten(),
            nn.Linear(self._get_out_features(in_chans, input_window_samples), n_classes),
            SqueezeFinalOutput(),
        )

    def _get_out_features(self, in_chans, input_window_samples):
        with torch.no_grad():
